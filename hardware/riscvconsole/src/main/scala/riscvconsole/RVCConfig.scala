...
class RVCPeripheralsConfig(gpio: Int = 14) extends Config((site, here, up) => {
  ...
  case riscvconsole.devices.klein.PeripheryKLEINKey => Seq(
    riscvconsole.devices.klein.KLEINParams(0x10004000))
  case riscvconsole.devices.blake2s.PeripheryBLAKE2SKey => Seq(
    riscvconsole.devices.blake2s.BLAKE2SParams(0x10005000))
  case riscvconsole.devices.chacha.PeripheryCHACHAKey => Seq(
    riscvconsole.devices.chacha.CHACHAParams(0x10006000))
